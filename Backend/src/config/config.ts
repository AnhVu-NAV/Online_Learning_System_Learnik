import dotenv from 'dotenv';

dotenv.config();

const JWT_SECRET = ''
const VIETQR_API_KEY = ''
// Định nghĩa các biến môi trường

const config = {
    MONGODB_URI: process.env.MONGODB_URI || 'mongodb://0.0.0.0:27017/education',
    VIETQR_API_KEY: process.env.VIETQR_API_KEY || "", // API key VietQR từ file .env
    JWT_SECRET: process.env.JWT_SECRET || JWT_SECRET,
    PORT: process.env.PORT || 4000
}

export default config